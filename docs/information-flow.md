...
| **R→T** | Candidate identities and match basis; exact stored content; source and time; current, historical, conflicting, or uncertain standing; relationship paths; query limits; validation failures; and per-effect write outcomes. Retry and reconciliation results also identify the prior operation, content match or mismatch, inspected target state, and each accepted, rejected, unchanged, or indeterminate effect. Relationship standing, such as provisional, remains separate from the persistence outcome. |
